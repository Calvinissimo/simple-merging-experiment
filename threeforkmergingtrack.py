...
class ThreeForkMergingTrack(Track):
    ...
    def closest_point_on_route(self, position):
        ...
        if position[1] > self._merge_point[1] and position[0] == self._merge_point[0]:
            before_or_after = 'after'
            track_side = TrackSide.CENTER
        else:
            before_or_after = 'before'

            if position[0] > self._track_width:
                # closest point is on right approach
                track_side = TrackSide.RIGHT
            elif position[0] < -self._track_width:
    ...
    def _closest_point_on_route_forced(self, position, track_side: TrackSide, before_or_after_merge):
        ...
        if before_or_after_merge == 'after' or track_side is TrackSide.CENTER:
